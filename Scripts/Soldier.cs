...
public abstract class Soldier : MonoBehaviour {
	...
	protected int tier;
	public Vector2 location; //MIGHT BE USELESS
	public int Tier {
		get {
			return tier;
		}
	...
	public void GetMultiSelected()
	{
		...
		//As multi soldier movement doesn't work perfectly with high number of soldiers, we kept limit of maximum soldiers selected to 10
		if (Selection.startLoc != -Vector3.one && render.color != Color.yellow && this.tag != "Enemy" && Selection.SelectedS.Count < 10) {
	...
	void CheckSurroundings ()
	{
		...
		if (objects.Length != 0) {
			...
			foreach (Collider2D c in objects )
			{
				...
				if(closest == null)
				{
					if(c.gameObject.tag == "Enemy"  && checkTargetSurrounding(c.gameObject))
				...
				//This object is closer that closest and its enemy
				        ...
				        && checkTargetSurrounding(c.gameObject))
				{
					closest = c.gameObject;
				}
			...
			if(ordered == -1 && closest != null && closest.tag == "Enemy")
			{
	...
	//Used for checking avaliabilty of target surrounding
	bool checkTargetSurrounding(GameObject target)
	{
		...
		if (alternatives.Count != 0) {
			do {
				...
				targetLoc = ((Tile)alternatives [alterIndex++]).WorldPosition;
				newTarget = grid.ConvertToTile (targetLoc);
			} while(grid.checkSingleTile(newTarget.X, newTarget.Y) && alterIndex != alternatives.Count);
			
			
		} 

		if (alterIndex == alternatives.Count){
			return false;
		}
	...
	void FixedUpdate () {
		...
		if(tag != "Enemy" && ordered == -1)
			CheckSurroundings ();

		//If there was a target, but died cancel order
		if (ordered == 1 && target == null)
			ordered = -1;
				...
				while (pathOrdered.Count > 0) {
					...
					if (grid.checkSingleTile (((Tile)checkPath.Peek ()).X, ((Tile)checkPath.Peek ()).Y)) {
						...
						if (grid.checkSingleTile(grid.ConvertToTile(point).X,grid.ConvertToTile(point).Y)) {
							...
							{
								do{
									point = ((Tile)alternatives [0]).WorldPosition;
...
*/
		...
		//Soldier is assigned to an enemy, target acquired
			...
			else 
			{
				if(counter == 0 || counter == 50){
					...
					//Rotate yourself to target
					float rot_z = Mathf.Atan2 ((target.transform.position - transform.position ).y
					                           , (target.transform.position - transform.position).x) * Mathf.Rad2Deg;
					//Rotate using the angle of rotation
					transform.localRotation = Quaternion.Euler (0f, 0f, rot_z - 90);


					//Enemy is dead, order completed
					if(Attack(target)){
						ordered = -1;
						target = null;
					}
	...
	protected abstract bool Attack(GameObject target);
	/*
